...
RUN if [ "$DEBUG_TOOLS" = "true" ] ; then \
    ...
    cp -f /kubernetes/client/bin/kubectl /eirini-ingress/binaries/; fi

FROM $BASE_IMAGE
COPY --from=build /eirini-ingress/binaries/* /bin/
ENTRYPOINT ["/bin/eirini-ingress"]
